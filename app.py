...
@app.route("/asaas/webhook", methods=["GET", "POST"])
def asaas_webhook():
    ...
    if not external_reference:
        return jsonify({
            "ok": True,
            "ignored": "missing externalReference",
            "event": event
        })
...
def create_payment():
    ...
    if not all([appointment_id, customer_name, value]):
        return jsonify({"ok": False, "error": "appointment_id, customer_name e value são obrigatórios"}), 400
